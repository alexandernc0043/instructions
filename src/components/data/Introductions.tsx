...
export default function Introductions() {
  ...
  const [imageCaption, setImageCaption] = useState(
    "At the beach on the eastern coast of Florida (2024)",
  );
  const [personalBackground, setPersonalBackground] = useState(
    "Grew up north of Charlotte, and have always had a love of computers.",
  );
  const [personalStatement, setPersonalStatement] = useState(
    "I’m a junior at UNC Charlotte studying Computer Science with a focus in Cybersecurity. I’m excited to collaborate and build secure, user‑friendly systems this semester.",
  );
  const [professionalBackground, setProfessionalBackground] = useState(
    "This is my first semester as an Instructional Assistant/Teachers Assistant, but before that I was a Peer Tutor for CCI.",
  );
  ...
  const [primaryComputer, setPrimaryComputer] = useState(
  ...
  const [courses, setCourses] = useState<Course[]>([
    {
      ...
      reason:
        "Required course for my concentration but the course’s subject also interests me.",
    },
    {
      dept: "ITIS",
      number: "3246",
      ...
      reason:
        "Another required course for my concentration but this is another class I am interested in.",
    },
    {
      dept: "THEA",
      number: "1512",
      ...
      reason:
        "I needed to take one more theme course and this one was recommended to me by a friend who took it previously.",
    },
    {
      dept: "MATH",
      number: "2164",
    ...
    },
    {
      ...
      reason:
        "I needed to take another science course and this was recommended to me by a friend.",
    },
    {
      dept: "BIOL",
      number: "1110L",
  ...
  const activeCourse = useMemo(
    () => (activeIndex == null ? null : (courses[activeIndex] ?? null)),
    [activeIndex, courses],
  );
  const resetDraft = () =>
    setDraft({ dept: "", number: "", name: "", reason: "" });

  // Compute JSON data and show it inline instead of downloading
  const inferredFilename =
    imageFilename || (image ? image.split("/").pop() || "image" : "image");
  const data = {
    firstName,
    preferredName,
    middleInitial,
    ...
    setShowJson((prev) => {
      ...
      if (!prev) {
        ...
        requestAnimationFrame(() => {
          jsonSectionRef.current?.scrollIntoView({
            behavior: "smooth",
            block: "start",
          });
        });
              ...
              <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    aria-label={
                      showJson
                        ? "Hide introduction JSON"
                        : "Show introduction JSON"
                    }
                    title={
                      showJson
                        ? "Hide your introduction data"
                        : "Show your introduction data as JSON"
                    }
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
                ...
                <div className="flex flex-col gap-1 col-span-full">
                  ...
                  <p className="text-xs text-neutral-500">
                    PNG, JPG, or GIF. Stays local in your browser.
                  </p>
                </div>
              ...
              <div>
                ...
                {courses.length === 0 && (
                  <p className="text-sm text-neutral-600">
                    No courses added. Use &#34;Add Course&#34; to include one.
                  </p>
                )}
                ...
                <div className="flex flex-col gap-3 sm:gap-4">
                  {courses.map((c, idx) => (
                    <div
                      key={idx}
                      className="border border-neutral-200 rounded-md p-3 sm:p-4 bg-white"
                    >
          ...
          <div className="my-6 bg-neutral-50 border border-neutral-200 rounded-lg p-4 sm:p-6 shadow-sm">
            <div className="space-y-4">
              <p className="text-sm text-neutral-700">
                I understand that what I put here is publicly available on the
                web and I won’t put anything here I don’t want the public to see{" "}
                {divider}{" "}
                {[
                  firstName?.trim()?.[0],
                  ...
                  .filter(Boolean)
                  .join("")}{" "}
                {divider} {new Date().toLocaleDateString()}
              </p>
              <h4 className="text-lg font-semibold">
                {firstName} {middleInitial}. &#34;{preferredName}&#34;{" "}
              ...
              <figure className={"m-auto w-100"}>
                ...
                <figcaption
                  className={
                    "flex justify-center mt-2 text-sm text-neutral-600"
                  }
                >
                  <em>{imageCaption}</em>
                </figcaption>
              </figure>
              {personalStatement && <p>{personalStatement}</p>}
              <ul className="list-disc pl-5 space-y-1">
                <li>
                  <strong>Personal Background: </strong>
                  {personalBackground === "" ? "None." : personalBackground}
                </li>
                <li>
                ...
                <li>
                  ...
                  {academicBackground === "" ? "None." : academicBackground}
                </li>
                <li>
                  <strong>Primary Computer: </strong>
                  {primaryComputer === "" ? "None." : primaryComputer}
                </li>
                <li>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start mt-2">
            <div className="md:col-span-2 flex flex-col gap-1">
              ...
              <input
                ...
                value={draft.dept}
                onChange={(e) =>
                  setDraft((prev) => ({ ...prev, dept: e.target.value }))
                }
                placeholder="ITIS"
                className="border border-neutral-300 rounded-md px-3 py-2 bg-white placeholder-neutral-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {activeCourse
                ? `Remove ${activeCourse.dept} ${activeCourse.number} — ${activeCourse.name}?`
                : "Remove this course?"}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      {showJson && (
        <section ref={jsonSectionRef}>
          ...
          <p className="text-sm text-neutral-600 mt-2">
            Copy this JSON into your site repository as needed.
          </p>
          <CodeHighlight>{JSON.stringify(data, null, 2)}</CodeHighlight>
        </section>
      )}
    </>
  );
